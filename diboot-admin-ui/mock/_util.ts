...
const resultJson = (code: number, msg: string, data?: any, ext = {}) =>
  Mock.mock({
    ...ext,
    code,
    data,
...
class JsonResult {
   ...
   * 服务不可用
   */
  FAIL_SERVICE_UNAVAILABLE(msg?: string) {
    return resultJson(5003, '服务不可用' + (msg ? `:${msg}` : ''))
  }
}

export default new JsonResult()
